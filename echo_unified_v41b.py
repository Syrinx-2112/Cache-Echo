#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# ECHO-UNIFIED v4.1 — corrections : as_completed, WAL, annulation propre, 
# purge cache au démarrage, JSONDecodeError, couleur Treeview, --version, code retour CLI.
"""
ECHO-UNIFIED v4.1 — Consolidation InternetDB
================================================================================
...
class Corpus:
    ...
    CLIENT_UA = f"echo-unified/{VERSION} (+internetdb-only)"
...
class InternetDBClient:
    ...
    def _throttle(self):
        # Le débit effectif global est plafonné à 1/min_interval requêtes par seconde,
        # quel que soit le nombre de workers, grâce à ce verrou partagé.
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                if e.code == 429 or 500 <= e.code < 600:
                    last_exc = e
                    # Back-off limité à 4s max pour éviter un blocage excessif
                    time.sleep(min(2 ** attempt, 4))
                    continue
...
class DeltaCalculator:
    ...
    def color_tk(delta: float) -> str:
        if delta < 0.25: return Corpus.C64['fg']
        if delta < 0.40: return Corpus.C64['fg_cyan']
        if delta < 0.55: return Corpus.C64['fg_yellow']
        if delta < 0.70: return Corpus.C64['fg_orange']
        if delta < 0.85: return Corpus.C64['fg_magenta']
...
class EchoScanner:
    ...
    def __init__(self, workers: int = 4, rate: float = 1.0,
        ...
        # [M2] Purge du cache au démarrage
    ...
    def _cancel_pending(self, futures: Dict[Future, str]):
        """Annule les futurs non encore démarrés pour une interruption propre."""
        for fut in futures:
    ...
    def scan(self, targets: List[str]) -> List[ScanResult]:
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as pool:
            ...
            try:
                # [H1] Itération as_completed pour un affichage au fur et à mesure
...
def export_results(results: List[ScanResult], path: str, fmt: str = "json"):
    # [M3] Création du dossier parent à l'export
...
if HAS_TK:
    ...
    class EchoUnifiedGUI:
        def __init__(self, root: tk.Tk):
            ...
            # [B1] self._batch_file supprimé (code mort)
            self._build_ui()
        ...
        def _build_ui(self):
            ...
            # [B3] Configuration des tags de couleur pour la Treeview
            self._tree.tag_configure("ok", foreground=Corpus.C64['fg'])
            self._tree.tag_configure("warn", foreground=Corpus.C64['fg_yellow'])
            self._tree.tag_configure("critical", foreground=Corpus.C64['fg_red'])
            
            det_frame = tk.LabelFrame(main, text=" [ DÉTAILS ] ", bg=C['bg'], fg=C['fg_white'],
                                      font=("Courier New", 8, "bold"))
            # [B7] Empêcher le redimensionnement automatique pour respecter la largeur
            det_frame.pack_propagate(False)
            det_frame.configure(width=380)
        ...
        def _gui_on_result(self, result: ScanResult):
            ...
            def _do():
                # [B3] Application des tags de couleur selon le delta
                if result.delta < 0.40:
                    tag = "ok"
                elif result.delta < 0.65:
                    tag = "warn"
                else:
                    tag = "critical"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # [M1] Groupe mutuellement exclusif pour les cibles
...
def main():
    ...
    # [B10] Récapitulatif des cibles rejetées en CLI
    if rejected:
    ...
    # [B6] Code de retour CLI cohérent
    if len(results) == 0:
        sys.exit(1)
    sys.exit(0)
